...
func NewServer(logger kitlog.Logger, config *todo.Configuration) *Server {
	svr := &Server{
		Logger:        logger,
		Config:        config,
	...
	if env.GetStringDefault("SWAGGER_ENABLE", "false") == "false" {
		docs.SwaggerInfo.Title = fmt.Sprintf("todo Backend [ AuthMode: %s ]", "Paseto")
		docs.SwaggerInfo.Description = "The Swagger Documentation For todo Backend API Server"
		docs.SwaggerInfo.Version = "1.0"
		docs.SwaggerInfo.Host = env.GetStringDefault("HOST_URL", "https://todo-be.liara.run")
	...
	svr.paseto, err = auth.NewPasetoMaker(env.GetStringDefault("JWT", ""))
	if err != nil {
		log.Fatal(err)
	}
...
func (s *Server) Listen() {
	...
	stop()
	log.Println("shutting down gracefully todo server, press Ctrl+C again to force")
	...
	log.Println("todo service exiting")
}
